...
(() => {
  ...
  function exportResults() {
    ...
    link.href=url; link.download='TIRI-v1-第四輪複檢結果.json'; document.body.append(link); link.click(); link.remove();
  ...
  $('import-file').addEventListener('change', async event => {
    ...
    try {
      ...
      if (parsed.version !== data.version || !Array.isArray(parsed.items)) throw Error('請選這一頁匯出的複檢 JSON；第三輪複檢結果已經帶入');
