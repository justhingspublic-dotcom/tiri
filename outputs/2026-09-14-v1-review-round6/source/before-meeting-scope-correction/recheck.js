...
(() => {
  ...
  $('delivery-notice').textContent = `${readyCount} 項已實際修改、可複檢${partialCount ? `；${partialCount} 項部分完成，限制已寫在卡片內` : ''}。${items.filter(item => item.reviewStatus === 'accepted').length} 項已通過紀錄與歷次留言完整保留；補件與名單確認另列於頁首，不影響這 6 項先行複檢。`;
  ...
  function exportResults() {
    const result = {
      clientConfirmations: data.clientConfirmations,
    ...
    link.href=url; link.download='TIRI-v1-第六輪複檢結果.json'; document.body.append(link); link.click(); link.remove();
