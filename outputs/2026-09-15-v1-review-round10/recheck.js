...
(() => {
  ...
  $('delivery-notice').textContent = '第 32 項與第 46 項本輪交付內容均已通過；46 英文正式說明等待客戶提供。其餘 44 項維持通過。';
  ...
  function exportResults() {
    const result = {
      ...
      sourceExportedAt:data.sourceExportedAt, sourceType:data.sourceType, sourceRecordedAt:data.sourceRecordedAt, contentDecision:data.contentDecision, additionalRequest:data.additionalRequest, pendingEnglishSource:data.pendingEnglishSource, exportedAt:timestamp(),
    ...
    link.href=url; link.download='TIRI-v1-第十輪複檢結果.json'; document.body.append(link); link.click(); link.remove();
