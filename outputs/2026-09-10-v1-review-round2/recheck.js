...
(() => {
  ...
  function refresh() {
    ...
    items.forEach(item => {
      ...
      const haystack = [item.id, item.title, item.currentChange, item.nextAction, item.deliveredChange, item.firstReview.note, item.previousRecheck.note, records[item.id]?.note].join(' ').toLocaleLowerCase('zh-TW');
  ...
  items.forEach(item => {
    ...
    card.innerHTML = `<div class="item-head"><span class="number">${item.id}</span><div class="title-group"><h2>${escapeText(item.title)}</h2>${item.currentFocus ? `<p class="current-focus">本輪：${escapeText(item.currentFocus)}</p>` : ''}<div class="meta-badges"><span class="badge ${item.deliveryStatus}">${deliveryLabels[item.deliveryStatus]}</span><span class="badge passed result" hidden></span></div></div></div><div class="body-grid"><div>${item.partialApproval ? `<p class="partial">${escapeText(item.partialApproval)}</p>` : ''}<h3>${item.reviewStatus === 'accepted' ? '此前已完成並通過的內容' : item.currentChange ? '這次已實際修改的內容' : item.reviewStatus === 'followup' ? '我的回覆／這次修改方向' : waiting ? '等待內容' : '已通過的內容'}</h3><p class="action-text">${escapeText(action)}</p>${item.currentCheck ? `<section class="current-check"><h3>這次請檢查</h3><p>${escapeText(item.currentCheck)}</p></section>` : ''}${item.deliveryLimit ? `<p class="delivery-limit"><strong>保留事項：</strong>${escapeText(item.deliveryLimit)}</p>` : ''}${item.assistantResponse ? `<section class="explanation"><h3>這次我的回覆</h3><p>${escapeText(item.assistantResponse)}</p></section>` : ''}${item.previousRecheck.note ? `<section class="original"><h3>你第一輪複檢的留言</h3><blockquote>${escapeText(item.previousRecheck.note)}</blockquote></section>` : ''}<details class="prior-work"><summary>第一輪複檢交付與確認紀錄</summary><p>${escapeText(item.reviewHistory[1].deliveredChange || '沿用第一輪已交付內容')}</p><p>${escapeText(item.reviewHistory[1].check || '')}</p><p>你的確認：${item.previousRecheck.confirmed ? '已通過' : '尚未通過'}</p>${item.reviewHistory[1].deliveryLimit ? `<p>當時保留事項：${escapeText(item.reviewHistory[1].deliveryLimit)}</p>` : ''}</details>${item.assistantExplanation ? `<section class="explanation"><h3>上一輪我的回覆與解釋</h3><p>${escapeText(item.assistantExplanation)}</p></section>` : ''}${item.firstReview.note ? `<section class="original"><h3>你第一輪驗收的原留言</h3><blockquote>${escapeText(item.firstReview.note)}</blockquote></section>` : ''}${!waiting ? `<details class="prior-work" open><summary>最初交付內容與檢查紀錄</summary>${item.reviewStatus === 'followup' ? `<p>${escapeText(item.deliveredChange)}</p>` : ''}<p class="prior-check">9/09 檢查紀錄：${escapeText(item.previousCheck)}</p></details>` : ''}${item.dependency ? `<p class="dependency">相關調整：${escapeText(item.dependency)}</p>` : ''}<div class="preview-links"><a class="open" target="_blank" rel="noopener" href="${escapeText(preview.href)}">開啟頁面檢查 ↗</a>${item.id === '08' ? '<a class="open" target="_blank" rel="noopener" href="../../v1/html/board.html?term=2022">已通過的第一、二屆 ↗</a>' : ''}</div></div><div class="review-form">${waiting ? '<p class="waiting-hint">這項仍等資料，可先在下方補充素材進度。</p>' : `<label class="check-label" for="confirm-${item.id}"><input id="confirm-${item.id}" type="checkbox">${item.reviewStatus === 'accepted' ? '維持通過' : item.deliveryLimit ? '本輪已修改部分通過' : '我已複檢，這項通過'}</label>${item.reviewStatus === 'accepted' ? '<p class="carry-hint">沿用你這次匯出的通過紀錄，需要再改可取消勾選。</p>' : ''}`}<label class="note-label" for="note-${item.id}">新的複檢意見</label><textarea id="note-${item.id}" rows="3" placeholder="還有需要調整的地方，可以寫在這裡。"></textarea></div></div>`;
  ...
  $('delivery-notice').textContent = `${readyCount} 項已實際修改、可複檢${partialCount ? `；${partialCount} 項部分完成，限制已寫在卡片內` : ''}。${acceptedIds.length} 項已通過紀錄與歷次留言完整保留；${waitingItems.length} 項仍等資料。`;
  ...
  function exportResults() {
    const result = {
      version:data.version, round:2, sourceVersion:data.sourceVersion,
      ...
      items:items.map(item => ({
        ...
        confirmationSource:typeof recordFor(item).confirmed === 'boolean' ? 'recheck' : item.reviewStatus === 'accepted' ? 'first-recheck' : 'not-reviewed',
        note:records[item.id]?.note || '', recheck:records[item.id] || null
      }))
    };
    const blob = new Blob([JSON.stringify(result,null,2)], {type:'application/json;charset=utf-8'});
    const url = URL.createObjectURL(blob); const link = document.createElement('a');
    link.href=url; link.download='TIRI-v1-第二輪複檢結果.json'; document.body.append(link); link.click(); link.remove();
  ...
  $('import-file').addEventListener('change', async event => {
    ...
    try {
      ...
      if (parsed.version !== data.version || !Array.isArray(parsed.items)) throw Error('請選這一頁匯出的複檢 JSON；第一輪複檢結果已經帶入');
