...
js = js[:start] + "  $('delivery-notice').textContent = '本輪修正 17 年份選單滾動；23、25 已於第七輪通過。44 項已通過、1 項可複檢，全部 45 項與歷次留言保留。';" + js[end:]
...
soup.select_one('.intro').string = '依第七輪結果修正年份選單的滾動問題。23 績效評估 Banner、25 公司治理 Banner 與流程圖效果已通過；目前 44 項通過，只剩 17 可複檢。'
for tag in soup.select('link[href*="recheck.css"],script[src*="recheck.js"]'):
    attr = 'href' if tag.name == 'link' else 'src'
    tag[attr] = tag[attr].replace('20260914-r7', '20260914-r8')
...
本輪修改一份共用 JavaScript 與兩個活動花絮頁面的資源版本參數。原圖檔未更動。
''')
