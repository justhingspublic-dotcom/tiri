...
(() => {
  ...
  items.forEach(item => {
    ...
    const extraLinks = (item.extraLinks || []).map(link => { const url = new URL('../../v1/html/' + link.route, location.href); url.searchParams.set('review', item.deliveryRevision); return `<a class="open" target="_blank" rel="noopener" href="${escapeText(url.href)}">${escapeText(link.label)} ↗</a>`; }).join('');
  ...
  $('delivery-notice').textContent = '第 46 項 The Icons 正式英文會員優惠已確認通過；目前全部 46 項均已通過。';
  ...
  function exportResults() {
    const result = {
      ...
      sourceExportedAt:data.sourceExportedAt, sourceType:data.sourceType, sourceRecordedAt:data.sourceRecordedAt, contentDecision:data.contentDecision, additionalRequest:data.additionalRequest, pendingEnglishSource:data.pendingEnglishSource, officialEnglishRequest:data.officialEnglishRequest, exportedAt:timestamp(),
    ...
    link.href=url; link.download='TIRI-v1-第十一輪複檢結果.json'; document.body.append(link); link.click(); link.remove();
