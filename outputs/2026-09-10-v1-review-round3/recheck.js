...
(() => {
  ...
  function exportResults() {
    const result = {
      ...
      items:items.map(item => ({
        ...
        confirmationSource:typeof recordFor(item).confirmed === 'boolean' ? 'recheck' : item.reviewStatus === 'accepted' ? 'second-recheck' : 'not-reviewed',
        note:records[item.id]?.note || '', recheck:records[item.id] || null
      }))
    };
    const blob = new Blob([JSON.stringify(result,null,2)], {type:'application/json;charset=utf-8'});
    const url = URL.createObjectURL(blob); const link = document.createElement('a');
    link.href=url; link.download='TIRI-v1-第三輪複檢結果.json'; document.body.append(link); link.click(); link.remove();
  ...
  $('import-file').addEventListener('change', async event => {
    ...
    try {
      ...
      if (parsed.version !== data.version || !Array.isArray(parsed.items)) throw Error('請選這一頁匯出的複檢 JSON；第二輪複檢結果已經帶入');
