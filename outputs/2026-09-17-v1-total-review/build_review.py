...
TEST_BASE = 'https://justhingspublic-dotcom.github.io/tiri/v1/html/'
...
for item in data['items']:
    ...
    previous_check = item.get('currentCheck') or item.get('previousCheck') or f'在 GitHub Pages 開啟「{item["title"]}」頁面，確認內容與互動符合最終版本。'
    ...
    item.update(
        ...
        currentFocus='GitHub Pages 全站總複檢',
...
data.update(
    ...
    testSite={
        ...
        'note': '所有頁面檢查連結均指向 GitHub Pages 預覽站。'
    }
)
...
js = js.replace("href=\"../../v1/html/board.html?term=2022\"", "href=\"https://justhingspublic-dotcom.github.io/tiri/v1/html/board.html?term=2022\"")
start = js.index("  $('delivery-notice').textContent =")
end = js.index('\n', start)
js = js[:start] + "  $('delivery-notice').textContent = 'GitHub Pages 總複檢：全部 46 項已重新開放；第 33、42 項已依本輪回報修正，請以最新內容檢查。前 11 輪紀錄均保留。';" + js[end:]
...
replacements = {
    '<title>TIRI v1｜第十一輪複檢</title>': '<title>TIRI v1｜GitHub Pages 總複檢</title>',
    ...
    '<h1>第十一輪複檢</h1>': '<h1>GitHub Pages 總複檢</h1>',
    '<p class="intro">第 46 項 The Icons 正式英文會員優惠已由使用者確認通過；中文版與其餘 45 項維持通過，目前全部 46 項均已完成驗收。</p>': '<p class="intro">這一頁重新開放全部 46 項，讓你在交付客戶前先用 GitHub Pages 逐項檢查。此前通過紀錄完整保留；本輪勾選會另外保存。</p>',
    ...
    '<div class="footer-links">': '<div class="footer-links"><a href="https://justhingspublic-dotcom.github.io/tiri/v1/html/index.html" target="_blank" rel="noopener">GitHub Pages 首頁 ↗</a><a href="../2026-09-16-v1-review-round11/第十一輪複檢.html" target="_blank" rel="noopener">第十一輪複檢頁 ↗</a>',
}
...
(OUT / 'README.md').write_text('''# TIRI GitHub Pages 總複檢

開啟 localhost 的「總複檢.html」，或雙擊「開啟複檢.command」。

- 全部 46 項重新開始，起始皆未勾選。
- 所有頁面檢查連結指向 `https://justhingspublic-dotcom.github.io/tiri/v1/html/`。
- 保留前十一輪交付、留言與通過紀錄。
...
- 保留前十一輪的通過、修改與留言歷史。
- 所有「開啟頁面檢查」連結改指向 `https://justhingspublic-dotcom.github.io/tiri/v1/html/`。
