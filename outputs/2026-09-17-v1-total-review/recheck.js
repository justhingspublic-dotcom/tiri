...
(() => {
  ...
  $('delivery-notice').textContent = 'GitHub Pages 總複檢：全部 46 項已重新開放；第 33、42 項已依本輪回報修正，請以最新內容檢查。前 11 輪紀錄均保留。';
  ...
  function exportResults() {
    const result = {
      ...
      sourceExportedAt:data.sourceExportedAt, sourceType:data.sourceType, sourceRecordedAt:data.sourceRecordedAt, contentDecision:data.contentDecision, additionalRequest:data.additionalRequest, pendingEnglishSource:data.pendingEnglishSource, officialEnglishRequest:data.officialEnglishRequest, testSite:data.testSite, exportedAt:timestamp(),
      items:items.map(item => ({
        id:item.id, title:item.title, currentFocus:item.currentFocus || '', deliveryStatus:item.deliveryStatus, changedThisRound:item.changedThisRound || false,
        ...
        currentChange:item.currentChange || '', currentCheck:item.currentCheck || '', deliveryLimit:item.deliveryLimit || '', route:item.route, extraLinks:item.extraLinks || [],
        ...
        confirmedAt:item.confirmedAt || null, confirmedRound:item.confirmedRound || null,
        note:records[item.id]?.note || '', recheck:records[item.id] || null
      }))
    };
    const blob = new Blob([JSON.stringify(result,null,2)], {type:'application/json;charset=utf-8'});
    const url = URL.createObjectURL(blob); const link = document.createElement('a');
    link.href=url; link.download='TIRI-v1-總複檢結果.json'; document.body.append(link); link.click(); link.remove();
